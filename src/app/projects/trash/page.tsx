...
export default async function TrashPage() {
  ...
  const isPrivileged = profile.role === 'admin' || profile.role === 'directeur'
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Corbeille des projets</h1>
            <p className="text-sm text-gray-500 mt-0.5">
              Restaurez ou supprimez définitivement les projets supprimés
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-100 rounded-xl text-sm text-amber-700">
        ...
        <p>
          Les projets dans la corbeille conservent toutes leurs données (tâches, documents, membres).{' '}
      ...
      <TrashTable isPrivileged={isPrivileged} />
    </div>
  )
}
