...
export async function POST(
  ...
  if (!pop.deleted_at)    return NextResponse.json({ error: 'Ce pop n\'est pas dans la corbeille' }, { status: 409 })

  const isPrivileged = ['admin', 'directeur'].includes(profile.role)
  const isAuthor     = pop.author_id === user.id
  if (!isPrivileged && !isAuthor) return NextResponse.json({ error: 'Permission refusée' }, { status: 403 })

  const { error: e1 } = await admin
    .from('pops').update({ deleted_at: null, deleted_by: null }).eq('id', id)

  if (!e1) return NextResponse.json({ ok: true, title: pop.content })

  const { error: e2 } = await admin
    .from('pops').update({ deleted_at: null }).eq('id', id)
